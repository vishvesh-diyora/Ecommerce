    ...
    <form class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4" @submit.prevent="search">
      ...
      <div class="db-field">
        <vue-select v-model="checkoutProps.form.category"
          class="db-field-control appearance-none cursor-pointer f-b-custom-select" id="customer" :options="categories"
          label-by="option" value-by="id" :closeOnSelect="true" :searchable="true" :clearOnClose="true"
          :placeholder="$t('label.select_category')" :search-placeholder="$t('label.search_category')"
      ...
      <div class="db-field">
        <vue-select v-model="checkoutProps.form.brand" class="db-field-control appearance-none cursor-pointer"
          ...
          :clearOnClose="true" :placeholder="$t('label.select_brand')" :search-placeholder="$t('label.search_brand')"
          @update:modelValue="setBrand($event)" />
      </div>
    </form>
  ...
  <div
          ...
          <div class="flex items-start justify-between gap-3">
            <div class="flex items-center gap-1 w-20 p-1 rounded-full bg-[#F7F7FC]">
              ...
              <input v-on:keypress="onlyNumber($event)" v-on:keyup="quantityUp(index, cart, $event)" type="number"
                :value="cart.quantity" class="text-center w-full h-5 text-sm font-medium">
    ...
    <div class="p-4">
      <div class="flex h-[38px] mb-4" v-if="carts.length > 0">
        ...
        <input v-model="discount" type="text" v-on:keypress="floatNumber($event)" :placeholder="$t('label.add_discount')"
