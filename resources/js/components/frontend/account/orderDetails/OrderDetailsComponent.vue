<template>
        ...
        <div class="row">
            <div class="col-12 md:col-5">
                ...
                <div class="rounded-2xl border border-gray-100"
                    ...
                    <h3 class="p-4 capitalize font-bold text-center"
                        v-if="address.address_type === enums.addressTypeEnum.BILLING">{{ $t('label.billing_address') }}</h3>
                    <ul class="p-4 flex flex-col gap-2.5 border-t border-dashed border-gray-100">
                        <li class="flex flex-wrap sm:flex-nowrap gap-2">
                            <span class="text-sm font-semibold capitalize w-20 flex-shrink-0">{{ $t('label.name') }}:</span>
                    ...
                    <ul class="border-b border-t border-dashed border-gray-100">
                        <li v-for="orderProduct in orderProducts" :key="orderProduct"
                            ...
                            <img :src="orderProduct.product_image" alt="product"
                                class="w-14 h-14 object-cover rounded-md flex-shrink-0" loading="lazy" />
    ...
    <div v-if="order.status !== enums.orderStatusEnum.CANCELED" class="flex flex-wrap gap-6 mobile:mb-20">
        ...
        <button v-if="order.status === enums.orderStatusEnum.PENDING" @click="changeStatus(enums.orderStatusEnum.CANCELED)"
            type="button"
    ...
    <div id="payment-modal" :class="Object.keys(route.query).length > 0 && route.query.status === 'success' && cart.length > 0 ? 'modal-active' : ''" class=" fixed inset-0 z-50 p-3 w-screen h-screen overflow-y-auto bg-black/50 transition-all duration-300 opacity-0 invisible">
        <div class="w-full rounded-xl mx-auto bg-white transition-all duration-300 max-w-[360px]">
            <div class="px-4 py-5 relative">
                ...
                <img :src="setting.image_confirm" alt="confirm-image" class="w-[120px] mx-auto mb-5" loading="lazy" />
